...
class TestJoinFunctionality:
    ...
    def department_df(self):
        ...
        return pd.DataFrame(
            {"id": [1, 2, 3], "department_name": ["Engineering", "HR", "Finance"]}
        )

    @pytest.fixture
    def partial_department_df(self):
        """Partial department dataframe for testing left/right joins"""
        return pd.DataFrame({"id": [1, 2], "department_name": ["Engineering", "HR"]})
    ...
    def test_multiple_joins(self):
        ...
        departments_df = pd.DataFrame({"id": [1, 2], "name": ["Engineering", "HR"]})

        managers_df = pd.DataFrame({"id": [1, 2], "name": ["Manager1", "Manager2"]})
    ...
    def test_cross_join(self, main_df, department_df):
        ...
        with List(small_main) as lst:
            ...
            result_lst = lst.run_query(
                joins=[{"type": Jointype.CROSS, "table": "departments"}]
            )
    ...
    def test_join_with_using_clause(self):
        ...
        with List(table1) as lst:
            ...
            result_lst = lst.run_query(
                joins=[{"type": Jointype.INNER, "table": "table2", "using": ["id"]}]
            )
    ...
    def test_anti_join(self):
        ...
        table2 = pd.DataFrame({"id": [2, 4], "status": ["inactive", "inactive"]})
    ...
    def test_semi_join(self):
        ...
        table2 = pd.DataFrame({"id": [2, 4], "status": ["active", "active"]})

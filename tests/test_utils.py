...
class DataGenerator:
    ...
    @staticmethod
    def create_data_with_nulls(
        n_rows: int = 10, null_percentage: float = 0.2
    ) -> pd.DataFrame:
...
class TestAssertions:
    ...
    @staticmethod
    def assert_statistical_value_close(
        actual: float, expected: float, tolerance: float = 0.001
    ):
...
class ListTestHelpers:
    ...
    def test_method_chaining(
        lst: List, operations: ListType[Dict[str, Any]]
    ) -> pd.DataFrame:
    ...
    @staticmethod
    def validate_list_state(
        lst: List, expected_shape: tuple = None, expected_columns: ListType[str] = None
    ):
        """Validate the current state of a List instance"""
        if expected_shape:
            assert (
                lst.data().shape == expected_shape
            ), f"Expected shape {expected_shape}, got {lst.data().shape}"

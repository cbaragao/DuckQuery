...
class TestIntegration:
    ...
    def test_complex_filtering_and_selection_workflow(self, titanic_sample):
        ...
        with List(titanic_sample) as lst:
            ...
            result_df = (
                lst.select(["survived", "pclass", "sex", "age"])
                .filter("sex = 'male'")
                .filter("survived = 1")
            ...
            with List(titanic_sample) as lst2:
                female_survivors = (
                    lst2.select(["survived", "pclass", "sex", "age"])
                    .filter("sex = 'female'")
                    .filter("survived = 1")
                    .order(["pclass ASC"])
                    .data()
    ...
    def test_join_workflow_with_additional_data(self, titanic_sample):
        ...
        with List(titanic_sample) as lst:
            ...
            assert all(
                name in enriched_data["class_name"].values
    ...
    def test_data_quality_analysis_workflow(self):
        ...
        messy_data = pd.DataFrame(
            {
                ...
                "age": [
                    ...
                    15,
                    14,
                ],  # 1000 is outlier, None is missing
        ...
        with List(messy_data) as lst:
            ...
            clean_data = lst.filter("age < 100").filter("age IS NOT NULL").data()
        ...
        sales_data = pd.DataFrame(
            {
                ...
                "sales": [
                    100 + i + (i % 7) * 20 for i in range(100)
                ],  # Trend with weekly pattern
    ...
    def test_comparative_analysis_workflow(self):
        ...
        with List(before_data) as lst:
            ...
            assert all(
                comparison["after_avg"] >= comparison["before_avg"]
    ...
    def test_complex_method_chaining_workflow(self, titanic_sample):
        ...
        with List(titanic_sample) as lst:
            ...
            result = (
                lst.select(["survived", "pclass", "sex", "age", "fare"])
                .filter("sex = 'female'")
            ...
            assert result["fare"].tolist() == sorted(
                result["fare"].tolist(), reverse=True
            )
